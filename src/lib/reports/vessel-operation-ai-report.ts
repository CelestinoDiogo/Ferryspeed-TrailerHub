...
const aiSectionsSchema = z.object({
  ...
  keyStatistics: z.string().trim().min(1),
  priorityHandling: z.string().trim().min(1),
  inspectionSummary: z.string().trim().min(1),
  temperatureCompliance: z.string().trim().min(1),
  damageSummary: z.string().trim().min(1),
  operationalIssues: z.string().trim().min(1),
  alertsGenerated: z.string().trim().min(1),
  exportActivity: z.string().trim().min(1),
  overallPerformance: z.string().trim().min(1),
  recommendations: z.string().trim().min(1),
});
    ...
    messages: [
      {
        ...
        content: [
          ...
          "For damageSummary, return exactly No damage was recorded for this vessel operation. when there are no damages.",
          "For operationalIssues, include only non-zero pending items. If none exist, return exactly All expected trailers have been accounted for and no inspection items remain outstanding.",
          "For temperatureCompliance, do not claim alerts when expected temperatures are not defined.",
          "Use compact markdown tables in inspectionSummary, damageSummary, and temperatureCompliance when helpful.",
          ...
          "Return JSON with these keys only: operationOverview, keyStatistics, priorityHandling, inspectionSummary, temperatureCompliance, damageSummary, operationalIssues, alertsGenerated, exportActivity, overallPerformance, recommendations.",
        ].join(" "),
      },
      {
        role: "user",
