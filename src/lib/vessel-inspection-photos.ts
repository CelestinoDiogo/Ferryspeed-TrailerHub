...
export const VESSEL_INSPECTION_PHOTO_BUCKET =
  "vessel-inspection-photos";

export const VESSEL_INSPECTION_PHOTO_INPUT_ACCEPT =
...
export type VesselInspectionPhotoDbCategory =
  (typeof VESSEL_INSPECTION_PHOTO_DB_CATEGORIES)[number];

const UI_TO_DB_CATEGORY: Record<
  string,
  VesselInspectionPhotoDbCategory
> = {
  "boat check": "inspection",
...
const resolveCategory = (
  ...
  return UI_TO_DB_CATEGORY[normalized] ?? "inspection";
};
...
export const saveVesselInspectionPhoto = async (
  ...
  if (
    !isUuidLike(vesselTrailerId) ||
    !isUuidLike(vesselOperationId)
  ) {
    throw new Error(
      "Photo upload requires a valid vessel trailer and vessel operation reference.",
    );
  }

  const normalizedTrailerNumber =
    normalizeTrailerNumber(input.trailerNumber);

  if (!normalizedTrailerNumber) {
    throw new Error(
      "Trailer number is required before uploading photos.",
    );
  }
  ...
  if (!ACCEPTED_MIME_TYPES.has(normalizedMime)) {
    throw new Error(
      "Only JPEG, PNG, or WebP files are supported.",
    );
  }
  ...
  const safeBaseName =
    sanitizeFileName(input.file.name || "photo") || "photo";
  ...
  const storagePath =
    `vessel-inspections/${vesselOperationId}/` +
    `${vesselTrailerId}/${Date.now()}-${uniqueToken}.${extension}`;

  const { error: uploadError } = await supabase.storage
    .from(VESSEL_INSPECTION_PHOTO_BUCKET)
  ...
  const payload = {
    ...
    uploaded_at: nowIso,
    uploaded_by:
      input.uploadedBy?.trim() || "TrailerHub User",
  };
 ...
 const { data, error: insertError } = await supabase
  ...
  if (insertError || !data) {
    ...
    if (cleanupError) {
      throw new Error(
        `${
          insertError?.message ||
          "Unable to save photo metadata."
        } Cleanup failed: ${
          cleanupError.message ||
          "Unable to remove orphaned file."
        }`,
      );
    }
  ...
  return {
  row: data as unknown as SavedVesselInspectionPhoto["row"],
  storagePath,
};
};