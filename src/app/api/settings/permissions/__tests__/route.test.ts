...
const importRoute = async () => import("@/app/api/settings/permissions/route");

const makeRequest = () =>
  new Request("http://localhost/api/settings/permissions", {
    method: "GET",
    headers: {
...
describe("GET /api/settings/permissions", () => {
  ...
  it("allows active administrator to read permissions", async () => {
    const { GET } = await importRoute();
    const response = await GET(makeRequest());

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      permissions: [{ roleKey: "administrator", moduleKey: "settings", canView: true }],
    });
  });
