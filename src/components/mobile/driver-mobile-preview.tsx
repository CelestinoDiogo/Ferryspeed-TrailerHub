...
export function DriverMobilePreview({ roleKey }: PreviewProps) {
  ...
  const grouped = useMemo(() => ({
    attention: tasks.filter((task) => !task.driverAcknowledgedAt || task.collectionAging?.level === "orange" || task.collectionAging?.level === "red" || instructions.some((instruction) => !instruction.readAt && instructionMatchesTask(instruction, task))),
    ...
    <PermissionGuard roleKey={roleKey} moduleKey="driver_mobile" action="view">
      <main className="min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#ecfeff_100%)] px-3 py-4 text-slate-950">
        <div className="mx-auto max-w-2xl space-y-4">
          ...
          {selectedDriver && overlayInstruction ? <div role="dialog" aria-modal="true" aria-label="Operational alert overlay" className={`fixed inset-0 z-[95] flex items-center justify-center px-4 ${overlayCritical ? "bg-rose-950/80" : "bg-slate-950/75"}`}><section className={`w-full max-w-lg rounded-3xl border-2 p-5 ${overlayCritical ? "border-rose-400 bg-rose-100 text-rose-950" : "border-amber-400 bg-amber-50 text-amber-950"}`}><p className="text-xs font-black uppercase tracking-[0.18em]">{overlayCritical ? "Critical" : "Attention"}</p><h2 className="mt-2 text-2xl font-black">New Instruction</h2><p className="mt-3 font-bold">{overlayInstruction.instruction}</p><p className="mt-5 w-full rounded-xl bg-slate-200 px-4 py-3 text-center text-sm font-black">READ-ONLY PREVIEW</p><button type="button" onClick={() => setDismissedAlertKeys((current) => [...current, `instruction:${overlayInstruction.id}`])} className="mt-2 w-full rounded-xl bg-slate-950 px-4 py-3 font-black text-white">Close Preview Alert</button></section></div> : null}
        </div>
      </main>
    </PermissionGuard>
  );
