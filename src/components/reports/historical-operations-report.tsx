    ...
    const load = async () => {
      ...
      try {
        if (kind === "departures") {
          ...
          setRecords(bookingRows.filter((row) => kind === "deliveries" || row.status === "waiting_collection" || row.status === "delivered" || row.status === "collected").map((row) => {
