...
import { filterHistoricalOperations, ownershipForTrailer, type HistoricalOperationKind, type HistoricalOperationRecord } from "@/lib/reports/historical-operations";
...
export function HistoricalOperationsReport({ kind }: { kind: HistoricalOperationKind }) {
  ...
  const filtered = useMemo(() => filterHistoricalOperations(records, { range, ownership, search }), [records, range, ownership, search]);
  const title = kind === "arrivals" ? "ARRIVALS REPORT" : "DEPARTURES REPORT";
  const label = kind === "arrivals" ? "Arrivals" : "Departures";
  ...
  const updateRange = (next: HistoryDateRangeValue) => {
    setRange(next);
    const params = new URLSearchParams(window.location.search);
    params.set("history", next.preset);
    if (next.preset === "custom") { params.set("start", next.startDate); params.set("end", next.endDate); } else { params.delete("start"); params.delete("end"); }
